...
| Image | `quay.io/<org>/oct-windows-builder:1.0.4-ocp4.22` (and `:1.0.4-ocp4.21`; `<semver>-ocp<major.minor>`) |
| Builder image | `quay.io/<org>/oct-windows-builder-builder:1.0.4-ocp4.22` (and `:1.0.4-ocp4.21`) |
...
Two axes in the catalog: git tag **`v1.x.x`** (semver) and optional branch **`ocp-X.Y`** when PatternFly or APIs diverge. Image tags **always** `<semver>-ocp<major.minor>` (e.g. `1.0.4-ocp4.22`).
