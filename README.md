...
- **Images:** `quay.io/<org>/oct-windows-builder:1.0.8-ocp4.22` / `:1.0.8-ocp4.21` and `quay.io/<org>/oct-windows-builder-builder:1.0.8-ocp4.22` / `:1.0.8-ocp4.21` (`<semver>-ocp<major.minor>`; same digest is OK)
...
After **public** combined tags exist for **both** OpenShift minors (`:1.0.8-ocp4.22` and `:1.0.8-ocp4.21` for the plugin and builder), open a PR against storefront `catalog/community.yaml` using [`catalog-tool.yaml`](catalog-tool.yaml) **including** `spec.versions[]`. Until then the Compute tile may exist without installable versions. Register `catalog/deploy/oct-windows-builder.yaml` in `BUNDLED_DEPLOY`. Always publish both minor tags. Never catalog a (version, OpenShift minor) row unless that exact tag is public.
